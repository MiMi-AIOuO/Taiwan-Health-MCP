...
class DrugService:
    def __init__(self, data_dir: str):
        ...
        # Cloud Run: disable schedulers / startup ETL to save memory (set env false).
        ...
        if self.enable_background_updates:
            ...
            self.scheduler = None
            log_info("DrugService: ENABLE_BACKGROUND_UPDATES=false, scheduler off")

        if self.enable_startup_update:
            self._check_startup_update()
        else:
            log_info("DrugService: ENABLE_STARTUP_UPDATE=false, skip startup ETL")
